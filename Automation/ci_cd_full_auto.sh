...
# NOTE: This check will fail if the key needs to be created, so it's safer to move this check
# after the key creation step, or assume the file exists if the key already exists.

# --- Create key pair if not exists ---
echo "🔑 Checking for existing key pair..."
if ! aws ec2 describe-key-pairs --key-names "$KEY_NAME" --region "$REGION" >/dev/null 2>&1; then
  echo "🆕 Creating new key pair and saving to $PEM_FILE..."
...
# Ensure the PEM file exists before proceeding (if key existed, it must be present locally)
if [ ! -f "$PEM_FILE" ]; then
  echo "❌ ERROR: PEM key file ($PEM_FILE) not found locally. Cannot proceed with SSH."
  exit 1
fi
...
# 🌟 FIX: Enable Auto-Assign Public IP on Subnet (THE CRITICAL STEP)
echo "🛠️ Configuring Subnet: Enabling MapPublicIpOnLaunch..."
aws ec2 modify-subnet-attribute \
  --subnet-id "$SUBNET_ID" \
  --map-public-ip-on-launch \
  --region "$REGION"
echo "✅ Subnet configured for auto-assignment of Public IPs."
# -----------------------------------------------------------------
...
echo "🔍 Fetching Public IP (may take up to 100 seconds)..."
MAX_RETRIES=10
SLEEP_SECONDS=10
INSTANCE_IP=""
...
  echo "❌ ERROR: Failed to get Public IP after several attempts. Check Subnet attribute."
  # You might want to add cleanup code here to tear down the VPC/resources
  exit 1
fi

# --- Install Apache and deploy website ---
echo "🧱 Configuring server and waiting for SSH to be ready..."

# 🌟 Improvement: Add an SSH connection retry loop to wait for sshd to start
for ((i=1; i<=5; i++)); do
    if ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 -i "$PEM_FILE" ubuntu@$INSTANCE_IP "exit" 2>/dev/null; then
        echo "✅ SSH connection established."
        break
    else
        echo "⏳ SSH not yet ready (attempt $i/5)..."
        sleep 5
    fi
done

if [ "$i" -gt 5 ]; then
    echo "❌ ERROR: Failed to establish SSH connection after multiple attempts."
    exit 1
fi

# Now run the configuration commands
ssh -o StrictHostKeyChecking=no -i "$PEM_FILE" ubuntu@$INSTANCE_IP <<'EOF'
  # Ensure we can run the update/install commands successfully
  sudo apt update -y
